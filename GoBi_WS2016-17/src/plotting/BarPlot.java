...
public class BarPlot extends Plot {
	...
	public void plot() {
		RExecutor r = new RExecutor(
				generateCommand(ConfigReader.readConfig().get("output_directory") + this.filename + ".png"));
	...
	String generateCommand(String filename) {
		...
		command += String.format(
				"barplot(x,names.arg=y,col=rainbow(\"%s\"),las=2" + (logScaleY ? ",log=\"y\"" : "") + ");",
				this.pair.getKey().size());
