...
public class ReadSimulator {
	...
	/*
	 * contains fragment length as key and amount of fragments having this
	 * length as value
	 */
	private TreeMap<Integer, Integer> fragLengthMap;
	/*
	 * contains amount mutations as key and amount of reads having this many
	 * mutations as value
	 */
	...
	public ReadSimulator(int readLength, double mean, double standardDeviation, double mutationRate,
		...
		ga = GTFParser.readGtfFile("h.ens.75",
				"/home/proj/biosoft/praktikum/genprakt-ws16/gtf/Homo_sapiens.GRCh37.75.gtf");

		gse = new GenomeSequenceExtractor(ch.getDefaultOutputPath() + "offsets.txt",
				"/home/proj/biosoft/praktikum/genprakt-ws16/assignment/a2/data/Homo_sapiens.GRCh37.75.dna.toplevel.fa",
				ga);
	}
	...
	private void simulateReads() {
		...
		simulMappingWriter.writeToWriter(
				"READ_ID\tCHR\tGENE_ID\tTRANSCRIPT_ID\tFW_REGVEC\tRW_REGVEC\tFW_REGVEC_TRANSCRIPT\tRW_REGVEC_TRANSCRIPT\tFW_MUT\tRW_MUT\n");
		...
		for (String[] array : readCountList) {
			...
			for (int i = 0; i < amount; i++) {
				...
				} while (fragmentLength <= readLength || fragmentLength > (transcriptLength - readLength)
						|| fragmentLength < 0);
				...
				for (int j = 0; j < readLength; j++) {
					buff += (FASTQQualityScores.QUALITY_SCORES_ARRAY[FASTQQualityScores.QUALITY_SCORES_ARRAY.length
							- 1]);
				}
				...
				int startInTranscriptRW = stop - readLength + 1; // !! stop -
																	// readLength
				int stopInTranscriptRW = stop; // !! stop-1

				Vector<Interval> genVecFW = currentTranscript.getGenomicRegionVector(startInTranscriptFW,
						stopInTranscriptFW);
				Vector<Interval> genVecRW = currentTranscript.getGenomicRegionVector(startInTranscriptRW,
						stopInTranscriptRW);
				...
				simulMappingWriter.writeToWriter(headerCount + "\t" + ga.getGene(geneId).getChromosome().getID() + "\t"
						+ geneId + "\t" + transcriptId + "\t" + genVecFWString + "\t" + genVecRWString + "\t"
						+ startInTranscriptFW + "-" + stopInTranscriptFW + "\t" + startInTranscriptRW + "-"
						+ stopInTranscriptRW + "\t" + StringUtils.join(fwMut, ", ") + "\t"
			...
			DebugMessageFactory.printInfoDebugMessage(ConfigReader.DEBUG_MODE,
					"Finished " + fin + "/" + readCountList.size());
			fin++;
		}
		...
		LinePlot lp = new LinePlot(pair, "Fragment Length Distribution", "Amount fragments", "Fragment length", minX,
				minY, maxX, maxY, false, false);
		lp.showLegend = false;
		...
		lp = new LinePlot(pair, "Mutation Distribution", "Amount mutations", "Amount reads", minX, 0, maxX, maxY, false,
				true);
		lp.showLegend = false;
	...
	public static void main(String[] args) {
		...
		ReadSimulator rsim = new ReadSimulator(75, 200.0, 80.0, 0.01,
				"/home/proj/biosoft/praktikum/genprakt-ws16/assignment/a2/data/readcounts.simulation");
